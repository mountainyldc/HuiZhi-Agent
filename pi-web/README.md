...
```bash
# 1. 克隆并安装（Node.js >= 22.19）
git clone https://github.com/agegr/pi-web.git
cd pi-web
npm install

# 2. 启动（首次先 build 再 start，或直接 dev）
npm run build
...
1. 打开 http://127.0.0.1:30141
2. 顶部打开 **Models** 面板 -> **Add provider** 添加 DeepSeek：
   - Base URL：`https://api.deepseek.com`
   - API Key：填你的 `DEEPSEEK_API_KEY`
   - 模型：`deepseek-chat`（或 pi 已识别出的 `deepseek-v4-pro`）
3. 在模型下拉里选中 DeepSeek 模型，新建会话即可对话
4. 让 Pi 跑商机雷达全流程：
   - 左侧 `Select project...` 选择 `fx-lead-radar` 仓库目录
   - 输入 **`跑一遍企业外汇需求商机雷达`**（或 `/radar`，注意第一次回车是补全，需再按一次发送）
   - 流程跑完后另开终端 `python engines/serve.py` 打开 http://127.0.0.1:8000 查看商机队列

> 模型配置写入 pi agent 目录（`~/.pi/agent/models-store.json`），与 pi CLI 共用。
